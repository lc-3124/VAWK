...
Style 的 `bg_sgr` 默认值为空字符串，即背景透明（而非黑色）。
不设定 `bg_sgr` 时，终端原生背景色会透出，支持半透明终端背景。
如需实色背景，显式传入：
...
void importFrame(PositionArgs offset,
                 const Framebuffer& src);  // 叠放另一帧缓冲
```
...
将另一个 Framebuffer 作为来源叠放到当前缓冲区上，
支持按格的透明通道独立控制合并方式。
...
`offset` 指定来源缓冲区的左上角在当前缓冲区中的位置。
超出边界的部分被裁剪。

透明合并规则（由来源格的 sgr_id_ 高 2 位决定）：

| 来源 fg | 来源 bg | 行为 |
|---------|---------|------|
| 设定 | 设定 | 使用来源的 fg、bg、字符，完全覆盖 |
| 设定 | 透明 | 使用来源的 fg 和字符，保留目标的 bg |
| 透明 | 设定 | 使用来源的 bg，保留目标的 fg 和字符 |
| 透明 | 透明 | 跳过该格，目标格不变 |

详见 `test/tui/cover.cpp` 的四象限演示。
...
struct Style {
    ...
    std::string bg_sgr      = "";            // 背景 SGR（默认透明）
    std::string effects_sgr;                 // 特效 SGR（默认无）
};
```

`bg_sgr` 默认值为空字符串——这意味着背景是**透明的**，
终端的原生背景色会透出。如需实色背景，显式传入 `bg(Color4::RED)` 等。
...
`vaterm::Trans` 是一个空标签类型，用于显式声明某一通道透明：

```cpp
#include <vaterm/enums.hpp>   // vatui.hpp 已包含

fg(TRANS)   // 返回空字符串 ""，表示前景透明
bg(TRANS)   // 返回空字符串 ""，表示背景透明
```

典型用法——让背景透明但设定前景色：

```cpp
Style{.fg_sgr = fg(Color4::BRIGHT_WHITE), .bg_sgr = bg(TRANS)}
```

这种风格写入的 Cell 具有 `has_fg=true, has_bg=false` 标记。
当通过 `importFrame` 叠放到另一个 Framebuffer 上时，该 Cell 的
背景会透出下层（目标格）的背景色，而前景和字符使用本格的值。

4 种可能的组合：

| fg | bg | importFrame 行为 |
|----|----|------------------|
| set | set | 完全覆盖下层（opaque） |
| set | TRANS | 保留下层背景色，使用本格前景和字符 |
| TRANS | set | 保留下层前景和字符，使用本格背景色 |
| TRANS | TRANS | 跳过该格，下层不变（hole） |
...
void   Framebuffer::importFrame(PositionArgs, const Framebuffer&);
